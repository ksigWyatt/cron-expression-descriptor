...
namespace CronExpressionDescriptor
{
    ...
    public class ExpressionDescriptor
    {
        ...
        private readonly string[] m_24hourTimeFormatTwoLetterISOLanguageName = new string[] {
            "ru",
            "uk",
            "de",
            "it",
        ...
        protected string GetFullDescription()
        {
            ...
            try
            {
                ...
                if (dayOfWeekDesc == "Weekdays")
                {
                    dayOfMonthDesc = string.Empty;
                }
                
                if (timeSegment.ToLowerInvariant().Contains("every minute"))
                {
                ...
                description = TransformVerbosity(description, m_options.Verbose).Replace("  ", " ").Replace("   ", " ").TrimStart(new char[] { ' ', ',' }).Trim().FirstCharToUpper();
            }
            catch (Exception ex)
            {
        ...
        protected string GetTimeOfDayDescription()
        {
            ...
            else
            {
                ...
                {
                    description.Append(", ");
                    minutesDescription = minutesDescription.Trim();
                }
                ...
                {
                    description.Append(", ");
                    hoursDescription = hoursDescription.Trim();
                }
        ...
        {
            string description = GetSegmentDescription(m_expressionParts[4], string.Empty,
               (s => new DateTime(DateTime.Now.Year, Convert.ToInt32(s),
                  1).ToString("MMMM", m_culture)),
        ...
        {
            string description = GetSegmentDescription(m_expressionParts[6], string.Empty,
               (s => Regex.IsMatch(s, @"^\d+$") ?
            ...
            Func<string, string> getRangeFormat)
        {
            ...
            else if (expression.Contains("/"))
            {
                ...
                {
                    string rangeItemDescription = string.Format(getDescriptionFormat(segments[0]), 
        ...
        protected string GenerateBetweenSegmentDescription(string betweenExpression, Func<string, string> getBetweenDescriptionFormat, Func<string, string> getSingleItemDescription)
        {
            ...
            if (betweenSegments.Count() == 2 && betweenSegments[0] == "1" && betweenSegments[1] == "5")
            {
                return "weekdays";
            }
            ...
            if (betweenSegment1Description == betweenSegment2Description)
            {
                return $"after {betweenSegment1Description}";
            }
        ...
        protected string FormatTime(string hourExpression, string minuteExpression, string secondExpression)
        {
            ...
            string response = string.Format("{0}{1}{2}{3}", hour.ToString(), minute, second, period);
